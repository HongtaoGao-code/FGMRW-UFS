...
def remove_constant_columns(data):
    ...
    non_constant_columns = np.any(data != data[0, :], axis=0)  # Check if there are any differences in the column
    return data[:, non_constant_columns]


def remove_constant_rows(data):
    # Remove rows where all values are the same
    non_constant_rows = np.any(data != data[0, :], axis=1)  # Check if there are any differences in the row
    return data[non_constant_rows, :]


def getGranularBall2(data):
    n, m = data.shape
    index = np.array(range(n)).reshape(n, 1)  # Create an index column, starting from 0
    data_index = np.hstack((data, index))  # Add index to data
    gb_list_temp = [data_index]
